      ...
      try {
        ...
        for (const integration of connectedIntegrations) {
          ...
          ;(identities || []).forEach((id) => {
            ...
            if (id.phone_jid && id.canonical_phone)
              identityMap.set(id.phone_jid, id.canonical_phone)
          })
          ...
          if (missingJids.length > 0) {
            ...
            for (let i = 0; i < newContacts.length; i += 50) {
              ...
              const { data: inserted } = await supabaseClient
                .from('whatsapp_contacts')
                .upsert(chunk, { onConflict: 'user_id,instance_id,remote_jid' })
                .select('id, remote_jid, phone_number')
              if (inserted) {
                inserted.forEach((c) => {
                  if (c.remote_jid) contactMap.set(c.remote_jid, c.id)
              ...
              const mappedMessages = allMessages
                .map((m: any) => {
                  ...
                  return {
                    ...
                    text,
                    type: m.message
                      ? Object.keys(m.message).filter((k) => k !== 'messageContextInfo')[0] ||
                        'text'
                      : m.messageType || 'text',
                    timestamp,
                    raw: m,
