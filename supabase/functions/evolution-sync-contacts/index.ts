...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const connectedIntegrations =
      integrations?.filter((i) => i.status === 'CONNECTED' || i.instance_name) || []

    if (integrationError || connectedIntegrations.length === 0) {
      throw new Error('No connected integrations found')
    }
          ...
          for (const c of validChats) {
            ...
            if (canonicalPhone) {
              ...
              if (existingIdentity) {
                ...
                if (phoneJid && existingIdentity.phone_jid !== phoneJid)
                  updates.phone_jid = phoneJid
            ...
            if (matches.length > 0) {
              ...
              if (
                lastMsgAt &&
                (!primary.last_message_at ||
                  new Date(lastMsgAt) > new Date(primary.last_message_at))
              ) {
            ...
            if (totalProcessed % 10 === 0) {
              await supabaseClient
                .from('import_jobs')
                .update({ total_items: totalChats, processed_items: totalProcessed })
                .eq('id', job.id)
            }
          }
        }
