...
// Limpa contatos mock (sem remote_jid) e dispara a sincronização das conversas
// reais da instância WhatsApp conectada.
//
// POST /backend/v1/whatsapp/cleanup-sync  { integrationId?: string }
//   → remove contatos sem remote_jid (mocks) + suas mensagens
//   → em seguida importa as conversas reais via Evolution API
//   → retorna contagem de remoções e importações
//
// Idempotente: seguro chamar várias vezes. Contatos reais são preservados.

routerAdd(
  'POST',
  '/backend/v1/whatsapp/cleanup-sync',
  (e) => {
    ...
    // ------------------------------------------------------------------
    // 1. Limpeza de contatos mock (sem remote_jid) + suas mensagens
    // ------------------------------------------------------------------
    ...
    try {
      ...
      for (let i = 0; i < mockContacts.length; i++) {
        ...
        // Exclui mensagens vinculadas a este contato mock.
        try {
      ...
      console.log('[cleanup-sync] erro na limpeza: ' + err)
    }

    // ------------------------------------------------------------------
    // 2. Localiza a integração conectada
    // ------------------------------------------------------------------
    let integ
    if (integrationId) {
      try {
      ...
      try {
        ...
        // Sem integração conectada: retorna só o resultado da limpeza.
        return e.json(200, {
          success: true,
          ...
          imported_messages: 0,
          sync_skipped: true,
          message: 'Limpeza concluída. Nenhuma integração conectada para sincronizar.',
        })
      }
    }

    // ------------------------------------------------------------------
    // 3. Sincroniza conversas reais da Evolution API
    // ------------------------------------------------------------------
    ...
    if (!evoUrl || !evoKey) {
      return e.json(200, {
        ...
        imported_messages: 0,
        sync_skipped: true,
        message: 'Limpeza concluída. Evolution API não configurada.',
      })
    }
